...
(function () {
  ...
  function _openSharePanel(text, url, anchor) {
    ...
    // One copy action only: the whole message (hook text + link). This is what you
    // paste into WhatsApp/iMessage on desktop. No separate "link only" button — that
    // split was the confusing bit; the link is always included here.
    var cm = document.createElement('button'); cm.type = 'button';
    cm.className = 'share-opt share-opt-primary'; cm.textContent = 'Copy message';
    cm.addEventListener('click', function () { _copy(text + '\n' + url, cm, 'Copied — paste it in'); });
    p.appendChild(cm);
